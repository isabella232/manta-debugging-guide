...
// XXX These first few paragraphs should move to a more common area, and we
// should also discuss /var/log/manta/upload.

Like most Manta logs, the haproxy logs for activity since the top of the hour
are stored in the load balancer zone itself.  In each load balancer zone, the
live log file is `/var/log/haproxy.log`.  This log is rotated hourly and
uploaded into Manta under `/poseidon/stor/logs/haproxy/YYYY/MM/DD/HH`, with an
object named for the first 8 characters of the zone.

For example, consider a request at 2018-12-05T01:37Z that was handled by load
balancer zone `f6817865-10fb-416c-a079-47941ac2aab4`.  From 01:37Z until 02:00Z,
you'd find the log entry in `/var/log/haproxy.log` inside zone
`f6817865-10fb-416c-a079-47941ac2aab4`.  After 0200Z, you'll find the log entry
in Manta inside the object
...
that case, you need to scan all of them for a given time.  For requests in the
last hour, you can use `manta-oneach` to do this.  (As always, care must be
taken with `manta-oneach`.)  For historical requests, you can use a compute job
or fetch the relevant log entries and analyze them locally.

For example, to search for requests from IP address `10.1.2.3` in the top of the
current hour:

    manta-oneach --service=loadbalancer 'grep 10.1.2.3 /var/log/haproxy.log | tail`

Note that we pass the result to `tail` within each zone to avoid transferring an
enormous amount of data if there's a large number of matching log entries.

To run the same query for historical logs, you might do something like this:

    mfind -t o /poseidon/stor/logs/haproxy/2018/12/05/01 |
        mjob create -o -m 'grep 10.1.2.3 /var/log/haproxy.log'

You can adjust the `mfind` invocation as needed to scan a broader time range.
